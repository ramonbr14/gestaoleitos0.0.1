...
-- -----------------------------------------------------
-- Table `mydb`.`pacienteemleito`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`pacienteemleito` ;

CREATE TABLE IF NOT EXISTS `mydb`.`pacienteemleito` (
  `pelpaccodigo` INT NOT NULL,
  `pelletcodigo` INT NOT NULL,
  `peldataentrada` TIMESTAMP(8) NULL DEFAULT 'DEFAULT CURRENT_TIMESTAMP',
  `peldatasaida` VARCHAR(45) NULL,
  PRIMARY KEY (`pelpaccodigo`, `pelletcodigo`),
  INDEX `fk_paciente_has_leito_leito1_idx` (`pelletcodigo` ASC) VISIBLE,
  INDEX `fk_paciente_has_leito_paciente1_idx` (`pelpaccodigo` ASC) VISIBLE,
  CONSTRAINT `fk_paciente_has_leito_paciente1`
    FOREIGN KEY (`pelpaccodigo`)
    REFERENCES `mydb`.`paciente` (`paccodigo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_paciente_has_leito_leito1`
    FOREIGN KEY (`pelletcodigo`)
    REFERENCES `mydb`.`leito` (`letcodigo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
